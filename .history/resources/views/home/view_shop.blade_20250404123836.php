...
<section class="shop_section layout_padding">
    ...
    @foreach($categories as $category)
        <option value="{{ route('view_shop', ['category' => $category->id]) }}" 
    ...
    @endforeach
                    ...
                    <div>
                        <label>Sort By:</label>
                        <select class="form-control-sm" onchange="window.location.href=this.value">
    <option value="{{ route('view_shop', ['sort' => 'newest']) }}" {{ request('sort') == 'newest' ? 'selected' : '' }}>Newest</option>
    <option value="{{ route('view_shop', ['sort' => 'price_low']) }}" {{ request('sort') == 'price_low' ? 'selected' : '' }}>Price: Low to High</option>
    <option value="{{ route('view_shop', ['sort' => 'price_high']) }}" {{ request('sort') == 'price_high' ? 'selected' : '' }}>Price: High to Low</option>
</select>
                    </div>
                </div>
            </div>
        ...
        <div class="btn-box text-center mt-4">
            <a href="{{ route('view_shop') }}">View All Products</a>
        </div>
    </div>
</section>
@endsection